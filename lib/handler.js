...
async function resolveLidToPhone(conn, lidNumber) {
  ...
  try {
    ...
    const checkList = [...new Set([...ownerList, ...sudoList, botPn])]
      .filter(Boolean)
...
const NOISE_THRESHOLD = 60; // Messages
...
async function handler(conn, mek, m) {
  try {
    ...
    // --- Pause Bot Gate ---
    if (db.pausedUntil && Date.now() < db.pausedUntil) {
      const body = m.body || "";
      ...
      if (prefix) {
        ...
        if (["unpausedebot", "unpausebot"].includes(cmdName)) {
          const senderNum = normalizeNumber(m.sender);
          const ownerList = Array.isArray(config.OWNER_NUMBER) ? config.OWNER_NUMBER.map(normalizeNumber).filter(Boolean) : [];
          const sudoList = Array.isArray(db.sudo) ? db.sudo.map(normalizeNumber).filter(Boolean) : [];
          const isOwnerOrSudo = ownerList.includes(senderNum) || sudoList.includes(senderNum) || mek.key.fromMe;
          if (!isOwnerOrSudo) {
            return;
          }
        } else {
    ...
    if (m.isGroup && !mek.key.fromMe) {
      ...
      if (traffic.count > NOISE_THRESHOLD) {
        ...
        if (!db.bannedGroups[from]) {
          db.bannedGroups[from] = Date.now() + 10 * 60 * 1000; // 10 minutes temporary ban
          saveGlobal(db);
          await conn.sendMessage(from, { 
            text: `⚠️ *GROUP NOISE ALERT*\n\nThis group has been temporarily muted for 10 minutes due to excessive message velocity (${traffic.count} msg/min). This protection is required to prevent the bot from crashing.\n\n🔓 *To Unban:* Run \`.unbangroup\`` 
          });
          return;
        }
      }
    ...
    let isBannedGroup = false;
    if (db.bannedGroups && db.bannedGroups[from]) {
      const expiry = db.bannedGroups[from];
    ...
    if (isBannedGroup) {
      ...
      if (prefix) {
        const cmdName = body.slice(prefix.length).trim().split(" ").shift().toLowerCase();
        if (["unbangroup", "unbangc"].includes(cmdName)) {
          const senderNum = normalizeNumber(m.sender);
          const ownerList = Array.isArray(config.OWNER_NUMBER) ? config.OWNER_NUMBER.map(normalizeNumber).filter(Boolean) : [];
          const sudoList = Array.isArray(db.sudo) ? db.sudo.map(normalizeNumber).filter(Boolean) : [];
          const isOwnerOrSudo = ownerList.includes(senderNum) || sudoList.includes(senderNum) || mek.key.fromMe;
    ...
    if (db.bannedGroups && db.bannedGroups[from] && !(isOwner && ["unbangroup", "unbangc"].includes(command))) return;
